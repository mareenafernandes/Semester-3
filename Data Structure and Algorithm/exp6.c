...
int count_noof_leaf(struct node * root)
{           
 if(root==NULL)
  return 0;
...
void inorder_without_stack(struct node * root)
{      
 if(root!=NULL)
{
...
void preorder_without_stack(struct node * t)
{        
  if(t==NULL)
  return;
  printf("%d\t",t->data);
...
void postorder_without_stack(struct node *t)
{        
    if(t==NULL)
    return;
